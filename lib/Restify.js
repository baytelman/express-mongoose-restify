...
exports.getModel = (model, options) => (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    const obj = convertModelToRest(model, yield query, options);
    res.json(obj);
});
...
exports.restifyModel = (router, model, { primaryKey, populate, requestHandler, methods, match, preprocessor }) => {
    if (!methods || methods.list) {
        if (requestHandler) {
            router.route('/').get(requestHandler, exports.listModel(model, { primaryKey, populate }));
        }
        else {
            router.route('/').get(exports.listModel(model, { primaryKey, populate }));
        }
    }
    ...
    if (!methods || methods.get) {
        if (requestHandler) {
            router.route('/:id').get(requestHandler, exports.getModel(model, { primaryKey, populate, match }));
        }
        else {
            router.route('/:id').get(exports.getModel(model, { primaryKey, populate, match }));
        }
    }
