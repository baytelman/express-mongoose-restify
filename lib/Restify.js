...
exports.listModel = (model, options, postprocessor) => (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    const all = yield Promise.all((yield query).map((c) => __awaiter(this, void 0, void 0, function* () { return convertModelToRest(model, yield postprocess(c, postprocessor), options); })));
...
exports.getModel = (model, options, postprocessor) => (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    const obj = yield model.findOne(matchCondition(id, options)).lean();
    res.json(convertModelToRest(model, yield postprocess(obj, postprocessor), options));
});
...
exports.restifyModel = (router, model, { primaryKey, requestHandler, methods, match, preprocessor, postprocessor }) => {
    if (!methods || methods.list) {
        if (requestHandler) {
            router.route('/').get(requestHandler, exports.listModel(model, { primaryKey }, postprocessor));
        }
        else {
            router.route('/').get(exports.listModel(model, { primaryKey }, postprocessor));
        }
    }
    ...
    if (!methods || methods.get) {
        if (requestHandler) {
            router.route('/:id').get(requestHandler, exports.getModel(model, { primaryKey, match }, postprocessor));
        }
        else {
            router.route('/:id').get(exports.getModel(model, { primaryKey, match }, postprocessor));
        }
    }
