...
const removeReadOnlyProps = (model) => IGNORE_PROPS_EDIT.forEach(prop => delete model[prop]);
const convertModelToRest = (model, obj) => {
    const schema = model.schema;
    return Object.keys(schema.paths).reduce((map, key) => {
        map[key] = obj[key];
        return map;
    }, { id: obj.id });
};
...
const matchCondition = (keyword, options) => {
    const condition = options && options.match ? { $or: options.match.map(field => ({ [field]: keyword })) } : { _id: keyword };
...
exports.restifyModel = (router, model, { preprocesor, methods, match }) => {
    ...
    if (!methods || methods.get) {
        if (preprocesor) {
            router.route('/:id').get(preprocesor, exports.getModel(model, { match }));
        }
        else {
    ...
    if (!methods || methods.put) {
        if (preprocesor) {
            router.route('/:id').put(preprocesor, exports.putModel(model, { match }));
            router.route('/:id').patch(preprocesor, exports.putModel(model, { match }));
        }
        else {
            router.route('/:id').put(exports.putModel(model, { match }));
            router.route('/:id').patch(exports.putModel(model, { match }));
        }
    }
    if (!methods || methods.delete) {
        if (preprocesor) {
            router.route('/:id').delete(preprocesor, exports.deleteModel(model, { match }));
        }
        else {
